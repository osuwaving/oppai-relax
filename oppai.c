    ...
    uint16_t n50, uint16_t nmiss, uint32_t score_version)
{
    ...
    /* high and low ar bonuses */
    if (mapstats.ar >= 10.87) {
        ar_bonus += 0.30 * (mapstats.ar - 10.87);
    } else if (mapstats.ar <= 10) {
        double low_ar_bonus = 0.01 * (10 - mapstats.ar);
	...
	/* hidden */
	if (mods & MODS_HD) {
		pp->aim *= 1 + pow(11 - mapstats.ar, 0.7) * 0.05;
	}

    /* flashlight */
    if (mods & MODS_FL) {
        pp->aim *= 1.18 * length_bonus;
    }
    ...
    if (mapstats.ar >= 10.87) {
    	acc_bonus *= 1 + 40 * pow(mapstats.ar - 10.87, 2.7);
	}
    ...
    pp->acc *= mymin(1.15, pow(ncircles / 500.0, 0.3));
